module.exports = {
  ...
  sendSmsLms: async function (mysql, db, req, resultList, sendType, splitMinutes, msgType) {
    ...
    const [rows,fields] = await dbPromise.query(sqlForSendResult, [id, count, userSendTitle, sendDate,sender, msg, sendType, msgType]);
  ...
  sendLms:async function(mysql, db, req, resultList, sendType, splitMinutes) {
    ...
    var sqlForSendResult = `INSERT INTO mmsSendResult VALUES(NULL,?,?,?,?,?,?,0,NULL,?)` //0: FILE_CNT / NULL : FILE_PATH1

    const dbPromise = db.promise();

    const [rows,fields] = await dbPromise.query(sqlForSendResult, [id, count, subject, sender, sendDate, msg, sendType]);
    var userSendIndex = rows.insertId;
    var sql = `INSERT INTO MMS_MSG (SUBJECT, PHONE, CALLBACK, STATUS, REQDATE, MSG, TYPE, mmsSendIndex)
    VALUES ('${subject}', ?,'${sender}', '0', '${sendDate}', '${msg}', '0', ${userSendIndex});`
